          ...
          <div className="backdrop-blur-sm bg-[#121A28]/40 p-5 sm:p-6 rounded-xl border border-gray-700/50">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">HTTPS monitor for http://example.com</p>
              </div>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mt-6">
              {[
                { label: "Current status", value: "Up", color: "text-green-400" },
                { label: "Last check", value: "26 seconds ago", color: "text-white" },
                { label: "Last 24 hours", value: "100%", color: "text-white" }
              ].map((stat, index) => (
                <div key={index} className="backdrop-blur-sm bg-[#0B0F17]/60 p-4 rounded-lg border border-gray-700/30">
                  <p className="text-gray-400 text-xs sm:text-sm mb-1">{stat.label}</p>
                  <p className={`${stat.color} font-semibold text-sm sm:text-base`}>{stat.value}</p>
                </div>
              ))}
            </div>

            <div className="mt-6 backdrop-blur-sm bg-[#0B0F17]/40 p-4 rounded-lg border border-gray-700/30">
              <p className="text-gray-400 text-sm mb-3 font-medium">Pulse stats</p>
              <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 text-sm">
                {[
                  ...
                  <div key={index}>
                    <p className="text-gray-400 text-xs sm:text-sm">{stat.period}</p>
                    <p className="text-green-400 font-semibold text-base sm:text-lg">{stat.uptime}</p>
                  </div>
                ))}
              </div>
            </div>
