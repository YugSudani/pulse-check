        ...
        <>
            ...
            <main className="flex-1 p-4 sm:p-6 md:p-8 mt-14 md:mt-0">
                ...
                <div className="relative flex flex-col sm:flex-row sm:flex-wrap gap-3 sm:gap-4 mb-6">
                    ...
                    />
                    {search && <span onClick={() => setSearch("")} className="absolute left-52 top-2 text-gray-400 cursor-pointer">clear</span>}

                    <select onChange={(e) => setCategory(e.target.value)} className="bg-[#131e30] px-4 py-2 rounded-lg hover:bg-[#1A2333] transition text-sm">
                ...
                <div className="grid grid-cols-2 gap-3 mb-6 lg:hidden">
                    ...
                    <div className="bg-[#131e30] p-4 rounded-xl border border-gray-800">
                        <h3 className="text-xs font-semibold mb-3 text-gray-300">Current status.</h3>
                        ...
                        <div className="grid grid-cols-3 gap-1 text-center text-xs">
                            <div>
                                <p className="text-gray-400 text-[10px]">Down</p>
                    ...
                    {/* Last 24 Hours Card - Compact Mobile Version */}
                    <div className="bg-[#131e30] p-4 rounded-xl border border-gray-800">
                        <h3 className="text-xs font-semibold mb-3 text-gray-300">Last 24 h.</h3>
                        
                        <p className="text-green-400 font-bold text-2xl mb-1">100%</p>
                        <p className="text-gray-400 text-[10px] mb-3">Overall uptime</p>
                        ...
                        <div className="space-y-1 text-[10px]">
                            ...
                            <p className="text-gray-400">1d Without incidents</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="h-135 sm:h-130 flex flex-col gap-4 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                    {[...monitors].reverse().map((monitor) => {
                        if(search && !monitor.name.toLowerCase().includes(search.toLowerCase())) return null;
                        if(category !== "all"){
                            if(category === "UP" &&  monitor.lastStatus !== "UP") return null;
                            if(category === "DOWN" &&  monitor.lastStatus === "UP") return null;
                            if(category === "PAUSED" &&  monitor.isActive == true) return null;
                        }

                    return(
                        <div key={monitor._id} onClick={() => navigate(`/monitor/${monitor._id}`, { replace: true })} className={`bg-[#131e30] p-4 sm:p-5 rounded-2xl relative cursor-pointer hover:bg-[#1A2333] transition  ${search ? "search-highlight" : ""}`}>
                            <h2 className="font-semibold text-base sm:text-lg mb-2">{monitor.name}</h2>
                            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 sm:gap-0">
                                <p className="text-gray-400 text-sm sm:text-base break-all sm:break-normal sm:flex-[7] sm:truncate">End Point : {monitor.url}</p>
                                <p className="text-gray-400 text-sm sm:text-base sm:px-2 sm:flex-[2]">Interval : {monitor.interval / 1000 / 60 > 0.99 ? monitor.interval / 1000 / 60 + " min" : monitor.interval / 1000 + " sec"}</p>
                                <div onClick={(e) => { e.stopPropagation(); toggleDeleteBox(monitor._id) }} className="absolute bottom-4 right-4 h-10 w-10 flex items-center justify-center hover:bg-[#2A3444] rounded-lg transition">
                                    <svg
                            ...
                            <div className="text-sm text-gray-400">{monitor.isActive ? "Active" : "Inactive"} / {monitor.lastStatus}</div>
                        </div>
                        )}
                    )}
                    {monitors?.length === 0 && (
