...
export default function ViewMonitor() {
    ...
    const fetchLogData = async (range) => {
        try {
            const response = await api.get(`/monitor/${id}/response-history?range=${range}`,
...
const handlePause = async (monitorId) => {
    try {
        await api.patch(`/monitor/pause/${monitorId}`, {},
            { withCredentials: true }
        );
    ...
    <div className="overflow-x-hidden overflow-y-auto  h-[99vh] bg-[#101724] text-white p-4 sm:p-6 md:p-8 lg:p-12 flex gap-6 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        ...
        <div className="flex-1 space-y-4 sm:space-y-5">
                ...
                <div className="flex justify-between">
                    <div className="flex flex-col gap-2">
                        ...
                        <p className="text-gray-400 w-65 md:w-120 text-xs sm:text-sm">
                            HTTPS monitor for{" "}
                            <a target="_blank" href={monitor?.url} className="text-green-500 hover:underline break-all">
                                {monitor?.url}
                            </a>
                        </p>
                    </div>
                    <div className="flex flex-col w-17 md:w-30 xs:flex-row items-stretch xs:items-center gap-2 flex-wrap">
                        <button onClick={() => navigate(`/edit-monitor/${id}`)} className="bg-[#131e30] px-4 py-2 w-17 md:w-30  rounded-lg text-xs sm:text-sm hover:bg-[#1A2333] cursor-pointer transition whitespace-nowrap">Edit</button>
                        <button className="bg-[#131e30] px-4 py-2 w-17 md:w-30 rounded-lg text-xs sm:text-sm hover:bg-[#1A2333] cursor-pointer transition whitespace-nowrap" onClick={() => handlePause(monitor._id)}>{monitorStatusBtn ? "Pause" : "Resume"}</button>
                        <button className="bg-[#131e30] px-4 py-2 w-17 md:w-30 rounded-lg text-xs sm:text-sm hover:bg-[#1A2333] cursor-pointer transition whitespace-nowrap">Test</button>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 w-[92vw] md:w-[60vw]">
                ...
                <div className="bg-[#131e30] border border-gray-800 rounded-xl p-4">
                    ...
                    <p className="font-bold text-lg sm:text-xl leading-tight tracking-wider">{monitor?.lastStatus ? <p className={`text-${monitor?.lastStatus === "UP" ? "green-400" : "red-400"}`}>{monitor?.lastStatus}</p> : <p>- - </p>}</p>
                    <p className="text-gray-400 text-xs mt-1">
                        Currently {monitor?.lastStatus === "UP" ? "up" : "down"} for {upDownTime?upDownTime:'- -'}
                    </p>
                </div>

                <div className="bg-[#131e30] border border-gray-800 rounded-xl p-4">
                    <p className="text-gray-400 text-xs sm:text-sm mb-1">Last check</p>
                    <p className="font-semibold text-sm sm:text-base">{timeAgo? timeAgo + ' ago' : 'Never checked'}</p>
                    <p className="text-gray-400 text-xs sm:text-sm mt-1">
                        Checked every {monitor?.interval / 1000 / 60 > 0.99 ? monitor?.interval / 1000 / 60 + " min" : monitor?.interval / 1000 + " sec"}
                    </p>
                    <p className="text-green-500 text-xs sm:text-sm mt-1">Net Pulse Count : {monitor?.totalChecks}</p>
                </div>

                <div className="bg-[#131e30] border border-gray-800 rounded-xl p-4">
                    <p className="text-gray-400 text-xs sm:text-sm mb-1">Last 24 hours</p>
            ...
            {/* ================= UPTIME SUMMARY ================= */}
            <div className="w-[93vw] md:w-[60vw] bg-[#131e30] border border-gray-800 rounded-xl p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">

                <div>
                    <p className="text-gray-400 text-xs sm:text-sm">Last 7 days</p>
                    <p className="text-green-400 font-semibold text-base sm:text-lg">100%</p>
                    <p className="text-gray-400 text-xs">0 incidents</p>
                </div>

                <div>
                    <p className="text-gray-400 text-xs sm:text-sm">Last 30 days</p>
                    <p className="text-red-400 font-semibold text-base sm:text-lg">81.099%</p>
                    <p className="text-gray-400 text-xs">6 incidents</p>
                </div>

                <div>
                    <p className="text-gray-400 text-xs sm:text-sm">Last 365 days</p>
                    <p className="font-semibold text-base sm:text-lg">--.--%</p>
            ...
            <div className="w-[93vw] md:w-[60vw] bg-[#131e30] border border-gray-800 rounded-xl p-4">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
                    <h2 className="font-semibold text-base sm:text-lg">Response time.</h2>
                    <select name="time" id="" onChange={(e) => setRange(e.target.value)} className="bg-[#2d3747] rounded-lg px-3 py-2 text-sm border-none cursor-pointer outline-none focus:outline-none focus:ring-0 w-full sm:w-auto">
                ...
                <div className="grid grid-cols-3 text-center text-xs sm:text-sm gap-2">
                    <div>
                        ...
                        <p className="font-semibold text-sm sm:text-base">{stats.avg !== null ? `${stats.avg} ms` : '-- ms'}</p>
                    </div>
                    <div>
                        <p className="text-gray-400">Minimum</p>
                        <p className="text-green-400 font-semibold text-sm sm:text-base">{stats.min !== null ? `${stats.min} ms` : '-- ms'}</p>
                    </div>
                    <div>
                        <p className="text-gray-400">Maximum</p>
                        <p className="text-red-400 font-semibold text-sm sm:text-base">{stats.max !== null ? `${stats.max} ms` : '-- ms'}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="w-[93vw] md:w-[60vw] bg-[#131e30] border border-gray-800 rounded-xl p-4">
                <div className="flex justify-between items-center mb-4 ">
                    ...
                    <button className="text-sm bg-[#1A2333] px-3 py-1 rounded-lg cursor-pointer" onClick={()=>triggerExportLog()}>
                        Export logs
                    </button>
                </div>
      ...
      <div className=" md:block bg-[#131e30] border border-gray-800 rounded-xl">
        <table className="text-sm w-full">
          ...
          <tbody>
            {[...incidents].reverse().slice(0, showIncidentCount).map((item, i) => (
              <tr
                key={i}
        ...
        {incidents.length === 0 && <p className="text-center text-gray-400 py-6">No Incidents found</p>}
            ...
            <div className="w-full flex p-4 justify-center justify-evenly text-sm gap-2">
                <button onClick={() => setShowIncidentCount(showIncidentCount + 5)} className="bg-[#1A2333] px-3 py-1 rounded-lg cursor-pointer">Load More</button>
                <button onClick={() => setShowIncidentCount(3)} className="bg-[#1A2333] px-3 py-1 rounded-lg cursor-pointer">Show Less</button>
            </div>
            </div>
