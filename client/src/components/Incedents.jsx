  ...
  return (
    <div className="bg-[#101724] text-white p-4 sm:p-6 md:p-10 overflow-y-auto flex-1 min-h-0 h-full w-full [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] ">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <h1 className="text-xl sm:text-2xl font-bold ml-12 mt-1.5 md:m-0">
          Incidents.
        </h1>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {[...incidents].reverse().map((item, i) => {
                if (category !== "all" && item.incidentType !== category)
                  return null;
                if (
                  search &&
                  !item.monitorUrl.toLowerCase().includes(search.toLowerCase())
                )
                  return null;
                ...
                return (
                  ...
                  >
                    <td
                      className={
                        "px-4 py-3 text-gray-200 max-w-xs truncate  " +
              ...
              <div className="text-xs text-gray-400 space-y-1">
                <div className="flex justify-between gap-2">
                  ...
                  <span className="text-right">
                    {new Date(item.incidentStartTime).toLocaleDateString(
                      "en-GB",
                    )}
                  </span>
                </div>
                <div className="flex justify-between gap-2">
                  <span>Resolved:</span>
                  <span className="text-right">
                    {new Date(item.incidentEndTime).toLocaleDateString("en-GB")}
                  </span>
                </div>
              </div>
            </div>
